...
--     minetest.log("action", "[SustainedDig] Digging at " .. minetest.pos_to_string(node_pos))    
--    end
--  end
-- end)
...
minetest.register_on_punchnode(function(pos, node, puncher, ...)
 if string.find(node.name, "tree") then
   -- Only reward if currently focused
   if current_look_pos and vector.equals(pos, current_look_pos) then
     set_reward_once(3.0, 0.0)
   end

   -- Also reward if actively aiming at it
   local look_dir = puncher:get_look_dir()
   local player_pos = vector.add(puncher:get_pos(), {x=0, y=1.5, z=0})
   local target = vector.add(player_pos, vector.multiply(look_dir, 10))

   for pointed in core.raycast(player_pos, target, false, false) do
     if pointed.type == "node" and vector.equals(pointed.under, pos) then
       set_reward_once(1.0, 0.0)
       break
     end
   end
 else
   set_reward_once(-0.2, 0.0)
 end
end)

-- minetest.register_on_punchnode(function(pos, node)
--   if string.find(node.name, "tree") and current_look_pos and vector.equals(pos, current_look_pos) then
--     set_reward_once(3.0, 0.0)
--   end  
-- end)
...
minetest.register_globalstep(function(dtime)
 ...
 for _, player in ipairs(minetest.get_connected_players()) do
   ...
   if ctrl and ctrl.LMB then
     ...
     local target = vector.add(eye_pos, vector.multiply(dir, 10))

     for pointed in core.raycast(eye_pos, target, false, false) do
       if pointed.type == "node" then
         local node_pos = pointed.under
         local node = minetest.get_node(node_pos)

         if node and string.find(node.name, "tree") then
           if dig_track.pos and vector.equals(dig_track.pos, node_pos) then
             -- continuing same dig
             dig_track.last_punch_time = now
             if now - dig_track.start_time > 1.5 then
               set_reward_once(0.5, 0.0)
               minetest.log("action", "[SustainedDig] Digging tree at " .. minetest.pos_to_string(node_pos))
               dig_track.start_time = now + 1000  -- prevent double-rewarding
             end
           else
             -- new target
             dig_track.pos = node_pos
             dig_track.start_time = now
             dig_track.last_punch_time = now
             dig_track.digging = true
           end
         else
           -- looking at non-tree
           dig_track.digging = false
         end
         break
       end
     end
   else
     -- Player stopped digging; check if dig was too short
     if dig_track.digging then
       local dig_duration = now - (dig_track.start_time or now)
       if dig_duration < 1.0 then
         set_reward_once(-2.0, 0.0)
         minetest.log("action", "[AbortedDig] Incomplete dig at " .. minetest.pos_to_string(dig_track.pos))
       end
       dig_track.digging = false
       dig_track.pos = nil
       dig_track.start_time = nil
     end
   end
 end
end)
...
--                 set_reward_once(-4.0, 0.0)  -- punish not finishing the dig
